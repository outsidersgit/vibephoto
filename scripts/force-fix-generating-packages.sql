-- Script SQL ULTRA SIMPLES para forçar correção de pacotes GENERATING travados
-- Execute este script no Supabase SQL Editor
-- ATENÇÃO: Este script é mais agressivo e força a atualização
...
-- 2. FORÇAR: Marcar todas as gerações PENDING/PROCESSING antigas como FAILED
UPDATE generations
SET 
...
WHERE status IN ('PENDING', 'PROCESSING')
  ...
  AND "updatedAt" < NOW() - INTERVAL '10 minutes'; -- Mais agressivo: 10 minutos
...
SET 
  ...
  "completedAt" = CASE
    WHEN EXISTS (SELECT 1 FROM generations g WHERE g."packageId" = up.id AND g.status = 'COMPLETED')
      AND NOT EXISTS (SELECT 1 FROM generations g WHERE g."packageId" = up.id AND g.status IN ('PENDING', 'PROCESSING'))
    THEN NOW()
    ELSE up."completedAt"
  END,
  "errorMessage" = CASE
