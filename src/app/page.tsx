...
const MarqueeCarousel = ({ items, hoveredIndex, onHoverChange, onImageClick }: MarqueeCarouselProps) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      translateXRef.current -= 1.6 // Velocidade de scroll (aumentada de 1.25 para 1.6)
    ...
    <div className="min-h-screen bg-white">
      ...
      {!session ? (
        <section className="relative h-screen w-full overflow-hidden" style={{fontFamily: '"SF Pro Display", -apple-system, BlinkMacSystemFont, system-ui, sans-serif'}}>
          ...
          {/* Mobile Lead Capture - Overlaid at Bottom */}
          <div className="lg:hidden absolute bottom-0 left-0 right-0 z-20 px-4 pb-6">
            <div className="max-w-sm mx-auto space-y-3">
              {/* Lead Capture Card */}
              <div className="bg-black/80 backdrop-blur-md rounded-xl shadow-2xl p-4 border border-white/30">
                <form 
                  onSubmit={async (e) => {
                    e.preventDefault()
                    const form = e.target as HTMLFormElement
                ...
                >
                  <input
                    ...
                    placeholder="Digite seu e-mail..."
                    required
                    className="w-full px-3.5 py-2.5 text-sm border border-gray-600 rounded-lg bg-black/50 text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                  />
                  <Button
                    type="submit"
                    size="sm"
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="w-full mt-2.5 bg-white/10 text-white border border-white/20 hover:bg-white/20 px-4 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 flex items-center justify-center gap-2"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              ...
              <Button
                variant="outline"
                size="lg"
                className="w-full border-2 border-white text-white bg-transparent hover:bg-white hover:text-black px-6 py-3 text-base font-medium rounded-lg backdrop-blur-sm hover:shadow-lg transition-all duration-200"
                onClick={() => {
                  const gallerySection = document.querySelector('#gallery-section');
                  if (gallerySection) {
                    gallerySection.scrollIntoView({ behavior: 'smooth' });
            ...
            <div className="bg-gradient-to-r from-blue-500 to-purple-600 px-6 py-4 text-white">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-semibold">
                  {activeToolModal === 'skin' && 'Upscale de Pele - Demonstração'}
