...
export function ModelCreationStep3FullBody({ modelData, setModelData, onNextStep, onPrevStep, canProceed = true }: ModelCreationStep3FullBodyProps) {
  ...
  const analyzePhotoQuality = async (files: File[], startIndex: number) => {
    ...
    try {
      ...
      console.log(`📸 [Step 3] Preparing ${files.length} images for analysis...`)
      const compressedFiles = await Promise.all(
        files.map(async (file, idx) => {
          try {
            console.log(`[Step 3] Processing image ${idx + 1}/${files.length}: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)}MB)`)
            const compressed = await compressImageIfNeeded(file, 4 * 1024 * 1024) // 4MB max for safety
            console.log(`[Step 3] Image ${idx + 1} ready: ${compressed.name} (${(compressed.size / 1024 / 1024).toFixed(2)}MB)`)
            return compressed
          } catch (error) {
            console.error(`❌ [Step 3] Failed to compress ${file.name}:`, error)
            alert(`Aviso: Não foi possível comprimir ${file.name}. A imagem pode ser muito grande para análise.`)
            return file
          }
        })
      )
      console.log(`✅ [Step 3] All ${compressedFiles.length} images prepared for analysis`)
